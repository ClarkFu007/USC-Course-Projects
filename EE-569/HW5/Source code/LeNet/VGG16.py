...
def normal_process(train_data, test_data):
    mean_value = np.mean(train_data, axis=(0, 1, 2, 3))
    std_value = np.std(train_data, axis=(0, 1, 2, 3))
    train_data = (train_data-mean_value)/(std_value+1e-7)
    test_data = (test_data-mean_value)/(std_value+1e-7)
