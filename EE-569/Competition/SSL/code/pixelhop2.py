...
# The "Pixelhop2" class inherits from the "cwSaab" class
class Pixelhop2(cwSaab):
    def __init__(self, depth=1, TH1=0.005, TH2=0.001, SaabArgs=None, shrinkArgs=None, concatArg=None):
        super().__init__(depth=depth, energyTH=TH1, SaabArgs=SaabArgs,
                         shrinkArgs=shrinkArgs, concatArg={'func': lambda X, concatArg: X})
        self.TH1 = TH1
        self.TH2 = TH2
        self.idx = []
...
if __name__ == "__main__":
    ...
    print(" -----> depth=1")
    p2 = Pixelhop2(depth=1, TH1=0.005, TH2=0.001, SaabArgs=SaabArgs,
                   shrinkArgs=shrinkArgs, concatArg=concatArg)
    p2.fit(X)
    output = p2.transform(X)

    print(" -----> depth=2")
    p2 = Pixelhop2(depth=2, TH1=0.005, TH2=0.001, SaabArgs=SaabArgs,
                   shrinkArgs=shrinkArgs, concatArg=concatArg)
    p2.fit(X)
    output = p2.transform(X)
