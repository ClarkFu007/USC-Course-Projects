...
class cwSaab():
    ...
    def __init__(self, depth=1, energyTH=0.01, SaabArgs=None, shrinkArgs=None, concatArg=None):
        ...
        assert (concatArg is not None), "Need parameter 'concatArg'!"
    ...
    def SaabTransform(self, X, saab, train, layer):
        ...
        if train == True:
            saab = Saab(num_kernels=SaabArg['num_AC_kernels'],
                        useDC=SaabArg['useDC'], needBias=SaabArg['needBias'])
            saab.fit(X)
    ...
    def cwSaab_1_layer(self, X, train):
        ...
        if self.SaabArgs[0]['cw'] == True:
            ...
            for i in range(X.shape[0]):
                X_tmp = X[i].reshape(S)
                if train == True:
                    saab, tmp_transformed, dc = self.SaabTransform(X_tmp,
                                                                   saab=None, train=True, layer=0)
                    saab_cur.append(saab)
                    eng.append(saab.Energy)
                else:
                    if len(saab_cur) == i:
                        break
                    _, tmp_transformed, dc = self.SaabTransform(X_tmp, saab=saab_cur[i],
                                                                train=False, layer=0)
        ...
        for i in range(len(saab_prev)):
            for j in range(saab_prev[i].Energy.shape[0]):
                ...
                if train == True:
                    ...
                    _, out_tmp, dc = self.SaabTransform(X_tmp, saab=saab_cur[pidx],
                                                        train=False, layer=layer)
                    pidx += 1
