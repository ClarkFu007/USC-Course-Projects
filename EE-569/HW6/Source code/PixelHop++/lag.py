...
class LAG():
    def __init__(self, learner, encode='onehot', num_clusters=[10, 10], alpha=5, par={}):
        assert (str(learner.__class__) == "<class 'llsr.LLSR'>"), \
            "Currently only support <class 'llsr.LLSR'>!"
    ...
    def compute_target_(self, X, Y, batch_size):
        ...
        for i in range(len(class_list)):
            ...
            if batch_size == None:
                kmeans = KMeans(n_clusters=self.num_clusters[i],
                                verbose=0, random_state=9, n_jobs=10).fit(
                    feature_train)
            else:
                kmeans = MiniBatchKMeans(n_clusters=self.num_clusters[i],
    ...
    def fit(self, X, Y, batch_size=None):
        ...
        assert (len(self.num_clusters) >= self.num_class), "'len(num_cluster)' must be " \
                                                           "larger than class number!"
        ...
        if self.encode == 'distance':  # this is the mode used in the paper
            ...
            for i in range(Yt.shape[0]):
                gt = Y[i].copy()
                dis = euclidean_distances(X[i].reshape(1, -1),
                                          self.centroid[self.clus_labels == gt]).reshape(-1)
...
if __name__ == "__main__":
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, digits.target,
                                                        test_size=0.2, stratify=digits.target)

    lag = LAG(encode='distance', num_clusters=[2, 2, 2, 2, 2, 2, 2, 2, 2, 2],
              alpha=5, learner=myLLSR(onehot=False))
